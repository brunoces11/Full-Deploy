---
name: deploy-full
description: Deploy static sites and single HTTP runtime applications from a local Codex workspace to a Docker + Traefik VPS, using an immutable confirmed plan, fixed source commit, isolated candidate validation, and rollback on failed public health checks.
---

# Deploy Full

Use this skill for Docker + Traefik deployments to the VPS. It supports static sites and Node, Python, or custom runtime applications, but it never guesses when the project cannot be classified safely.
...
`PERSISTENT_MOUNTS` uses `subdir:/app/path,other-subdir:/app/other-path`. The plan must show both the base directory `/compose/volume/PROJECT_NAME` and every host/container mount. Mount paths are checked for existence and writability during application.
...
3. It generates `Dockerfile.deploy` and the Docker-supported `Dockerfile.deploy.dockerignore` inside the candidate, never rewriting the project’s `.dockerignore`.
...
- No runtime app is successful unless it serves HTTP on the confirmed internal port.
